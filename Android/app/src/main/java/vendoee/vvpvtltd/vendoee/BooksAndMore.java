...
public class BooksAndMore extends Fragment {
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container, Bundle savedInstanceState) {
        rootView=inflater.inflate(R.layout.activity_books_and_more,container,false);
        myDB = new DatabaseHelper(getContext());
        notify = (TextView)rootView.findViewById(R.id.note1);
        gps = new GPSTracker(getContext());
        load = (RelativeLayout) rootView.findViewById(R.id.loadingPanel);
        recyclerView=(RecyclerView)rootView.findViewById(R.id.recycler_view6b);
        ...
        res = myDB.getItemDetails("Books & more");
        int c5 = res.getCount();
        if(res.getCount()>0){
            RecyclerView recyclerView=(RecyclerView)rootView.findViewById(R.id.recycler_view6b);
            ...
            public void onScrolled(RecyclerView recyclerView, int dx, int dy) {
                if (recyclerView.getAdapter().getItemCount() >= 3) {
                    ...
                    if ( loadin == false && lastVisibleItemPosition != RecyclerView.NO_POSITION && lastVisibleItemPosition == recyclerView.getAdapter().getItemCount() - 1)
                    {
                        ...
                        res = myDB.getItemDetailsfIX("Books & more",last_id,2);
                        int c = res.getCount();
                        if(res.getCount()>0){
                            if(checkForNext==true){
        ...
        StringRequest stringRequest = new StringRequest(Request.Method.POST, "http://vendoee.com/android-scripts/offerSales2EC.php",
                ...
                new Response.ErrorListener() {
                    ...
                    public void onErrorResponse(VolleyError error) {
                        ...
                        //Toast.makeText(getContext(),"ID Load BooksMore- "+ error.toString(),Toast.LENGTH_LONG).show();
                    }
                }){
            @Override
        ...
        StringRequest stringRequest = new StringRequest(Request.Method.POST, "http://vendoee.com/android-scripts/offerSales2E.php",
                new Response.Listener<String>() {
                    ...
                    public void onResponse(String response) {
                        ...
                        //Toast.makeText(rootView.getContext(),"here"+response,Toast.LENGTH_SHORT).show();
