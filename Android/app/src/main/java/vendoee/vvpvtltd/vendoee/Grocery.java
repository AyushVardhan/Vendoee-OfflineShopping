...
public class Grocery extends Fragment {
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container, Bundle savedInstanceState) {
        rootView=inflater.inflate(R.layout.activity_grocery,container,false);
        myDB = new DatabaseHelper(getContext());
        notify = (TextView)rootView.findViewById(R.id.note1);
        load = (RelativeLayout) rootView.findViewById(R.id.loadingPanel);
        recyclerView=(RecyclerView)rootView.findViewById(R.id.recycler_view7b);
        ...
        res = myDB.getItemDetails("Grocery");
        int c5 = res.getCount();
        if(res.getCount()>0){
            RecyclerView recyclerView=(RecyclerView)rootView.findViewById(R.id.recycler_view7b);
            ...
            public void onScrolled(RecyclerView recyclerView, int dx, int dy) {
                if (recyclerView.getAdapter().getItemCount() >= 3) {
                    ...
                    if ( loadin == false && lastVisibleItemPosition != RecyclerView.NO_POSITION && lastVisibleItemPosition == recyclerView.getAdapter().getItemCount() - 1)
                    {
                        ...
                        res = myDB.getItemDetailsfIX("Grocery",last_id,2);
                        int c = res.getCount();
                        if(res.getCount()>0){
                            if(checkForNext==true){
        ...
        StringRequest stringRequest = new StringRequest(Request.Method.POST, "http://vendoee.com/android-scripts/offerSales2EC.php",
                ...
                new Response.ErrorListener() {
                    ...
                    public void onErrorResponse(VolleyError error) {
                        ...
                        //Toast.makeText(getContext(),"ID Load Grocery- "+ error.toString(),Toast.LENGTH_LONG).show();
                    }
                }){
            @Override
