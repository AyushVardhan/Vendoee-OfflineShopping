...
public class Appliances extends Fragment {
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container, Bundle savedInstanceState) {
        rootView=inflater.inflate(R.layout.activity_appliances,container,false);
        ...
        res = myDB.getItemDetails("Appliances");
        int c5 = res.getCount();
        if(res.getCount()>0){
            recyclerView=(RecyclerView)rootView.findViewById(R.id.recycler_view1);
            //Toast.makeText(getContext(),"Appliances-GotData",Toast.LENGTH_SHORT).show();
            saleItemsList=new ArrayList<>();
            adapter=new SaleItemsAdapter(rootView.getContext(), saleItemsList);
            ...
            public void onScrolled(RecyclerView recyclerView, int dx, int dy) {
                if (recyclerView.getAdapter().getItemCount() >= 3) {
                    ...
                    if (  loadin == false && lastVisibleItemPosition != RecyclerView.NO_POSITION && lastVisibleItemPosition == recyclerView.getAdapter().getItemCount() - 1)
                    {
                        ...
                        res = myDB.getItemDetailsfIX("Appliances",last_id,2);
                        int c = res.getCount();
                        if(res.getCount()>0){
                            if(checkForNext==true){
        ...
        StringRequest stringRequest = new StringRequest(Request.Method.POST, "http://vendoee.com/android-scripts/offerSales2EC.php",
                    ...
                    public void onResponse(String response) {
                        ...
                        if (result.matches(str)) {
                            ...
                            while (str1.hasMoreTokens()) {
                                ...
                                //Toast.makeText(getActivity(),"NextCheck : "+nextCheck,Toast.LENGTH_SHORT).show();
                ...
                new Response.ErrorListener() {
                    ...
                    public void onErrorResponse(VolleyError error) {
                        //loading.dismiss();
                        //Toast.makeText(getContext(),"ID Load Appliances- "+ error.toString(),Toast.LENGTH_LONG).show();
    ...
    private void loadOffers(final String city) {
        ...
        StringRequest stringRequest = new StringRequest(Request.Method.POST, "http://vendoee.com/android-scripts/offerSales2E.php",
                new Response.Listener<String>() {
                    @Override
                    public void onResponse(String response) {
                        String str = "0"; weak.setVisibility(View.INVISIBLE);
                        String result = response;
                        if (result.isEmpty()) {
                ...
                new Response.ErrorListener() {
                    @Override
                    public void onErrorResponse(VolleyError error) {
                        //Toast.makeText(getContext(),"Load Appliances- "+ error.toString(),Toast.LENGTH_LONG).show();
