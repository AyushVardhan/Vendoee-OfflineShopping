...
public class Men extends Fragment {
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container, Bundle savedInstanceState) {
        rootView=inflater.inflate(R.layout.activity_men,container,false);
        ...
        res = myDB.getItemDetails("Men");
        int c5 = res.getCount();
        if(res.getCount()>0){
            RecyclerView recyclerView=(RecyclerView)rootView.findViewById(R.id.recycler_view2);
            //Toast.makeText(getContext(),"men-GotData",Toast.LENGTH_SHORT).show();
            saleItemsList=new ArrayList<>();
            adapter=new SaleItemsAdapter(rootView.getContext(), saleItemsList);
            ...
            public void onScrolled(RecyclerView recyclerView, int dx, int dy) {
                if (recyclerView.getAdapter().getItemCount() >= 3) {
                    ...
                    if (  loadin == false && lastVisibleItemPosition != RecyclerView.NO_POSITION && lastVisibleItemPosition == recyclerView.getAdapter().getItemCount() - 1)
                    {
                        ...
                        res = myDB.getItemDetailsfIX("Men",last_id,2);
                        int c = res.getCount();
                        if(res.getCount()>0){
        ...
        StringRequest stringRequest = new StringRequest(Request.Method.POST, "http://vendoee.com/android-scripts/offerSales2EC.php",
                ...
                new Response.ErrorListener() {
                    ...
                    public void onErrorResponse(VolleyError error) {
                        ...
                        //Toast.makeText(getContext(),"ID Load Men- "+ error.toString(),Toast.LENGTH_LONG).show();
                    }
                }){
            @Override
